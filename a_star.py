...
def aStar(matrix: List[List[int]], entry: Tuple[int, int], exit: Tuple[int, int]) -> [
    ...
    pathTracker = {}
    priorityQueue = []
    hq.heappush(priorityQueue, (0+h(entry, exit), 0, entry)) # stored like: (fScore, gScore, position)
    gScores = {entry: 0}
    ...
    while priorityQueue:
        ...
        for directionX, directionY in directions:
            ...
            if adjacentPos not in gScores or gScore+1 < gScores[adjacentPos]:
                ...
                hq.heappush(priorityQueue, (gScores[adjacentPos]+h(adjacentPos, exit), gScores[adjacentPos], adjacentPos))
